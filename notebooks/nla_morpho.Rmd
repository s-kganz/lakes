...
## Terrain Analysis
Given the large spatial extent of the data sources, we determined median slope in a 1km buffer region surrounding lakes with a digital elevation model generated at 90m cell size by the Shuttle Radar Topography Mission[@jarvisHolefilledSRTMGlobe2008]. All analysis was performed with Google Earth Engine[@gorelickGoogleEarthEngine2017a].
...
This equation asymptotically approaches $Z_{limit}$, an upper limit on maximum lake depth. At the shoreline, the profile 
...
mn_poles    <- st_read("data_working/mn_poles_inaccessbility.shp") %>%
  tibble() %>%
...
The prediction from the asymptotic model is far worse. Prediction for low depths is decent, but deeper lakes reach the asymptotic maximum. A more appropriate relationship may be logarithmic.

\begin{align}
$\hat{Z} = \ln{(S \times D_{max})}$
\end{align}

```{r log-est}
nla_log <- nla %>%
  mutate(
    zhat = log(slope50_p25 * dist_pole)
  ) %>%
  drop_na()

nla_log_lm <- lm(maxdepth ~ zhat, data=nla_asymp)
nla_log$zcorrected <- predict(nla_log_lm, nla_log)

ggplot(nla_log, aes(x=maxdepth, y=zcorrected)) +
  geom_abline(slope=1, intercept=0, color="red", size=2, linetype="dashed") + 
  geom_point() +
  coord_equal() + ylim(0, 60) +
  labs(y=TeX("\\hat{Z} (m)"), x=TeX("Z_{max} (m)")) 
